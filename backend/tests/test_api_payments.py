"""Payment ingestion retrieval: service + GET /api/v1/razorpay/payments.
...
response shape, including exact minor-unit fee/tax propagation.
"""
...
CREATED_AT_EPOCH = 1755907200  # 2025-08-23T00:00:00Z


def payment_payload(pid="pay_dummy0001", **overrides):
    payload = {
        "id": pid,
        "entity": "payment",
        "amount": 500_000,
...
class TestListPaymentsService:
    def test_successful_page_normalized(self) -> None:
        ...
        def handler(request: httpx.Request) -> httpx.Response:
            ...
            return httpx.Response(200, json=collection(payment_payload()))

        result = make_service(handler).list_payments(count=10, skip=0)
        assert len(result.items) == 1
        item = result.items[0]
        assert item.provider == "razorpay"
        assert item.external_id == "pay_dummy0001"
        assert item.amount_minor == 500_000
        assert item.currency == "INR"
        assert item.status == "captured"
        assert item.order_id == "order_dummy0001"
        assert item.method == "upi"
        assert item.captured is True
        assert item.fee_minor == 588
        assert item.tax_minor == 89
        assert item.created_at is not None and item.created_at.year == 2025
        # Raw wire fields that the application does not need must be dropped.
        assert "acquirer_data" not in item.model_dump()
    ...
    def test_sequential_pages_are_independent(self) -> None:
        def handler(request: httpx.Request) -> httpx.Response:
            ...
            if skip == 0:
                items = [payment_payload("pay_a"), payment_payload("pay_b")]
            else:
                items = [payment_payload("pay_c")]
            return httpx.Response(200, json=collection(*items))

        service = make_service(handler)
        first = service.list_payments(count=2, skip=0)
        second = service.list_payments(count=2, skip=2)
        assert [i.external_id for i in first.items] == ["pay_a", "pay_b"]
        assert [i.external_id for i in second.items] == ["pay_c"]

    def test_empty_collection(self) -> None:
        def handler(request):
            return httpx.Response(200, json=collection())

        result = make_service(handler).list_payments()
        assert result.items == [] and result.count == 0

    def test_uncaptured_payment_has_no_fee(self) -> None:
        payload = payment_payload(status="created", captured=False)
        payload.pop("fee"), payload.pop("tax")

        def handler(request):
            return httpx.Response(200, json=collection(payload))

        item = make_service(handler).list_payments().items[0]
        assert item.fee_minor is None and item.tax_minor is None
    ...
    def test_typed_failures_propagate(self, status, expected_exc):
        ...
        with pytest.raises(expected_exc):
            make_service(handler).list_payments()

    def test_out_of_bounds_count_rejected_locally(self) -> None:
        def handler(request):
            return httpx.Response(200, json=collection())

        with pytest.raises(RazorpayError):
            make_service(handler).list_payments(count=MAX_PAGE_SIZE + 1)
...
class TestPaymentsEndpointSuccess:
    def test_returns_normalized_payments(self, api, install_service) -> None:
        install_service(make_service(
            lambda request: httpx.Response(200, json=collection(payment_payload()))
        ))
        response = api.get("/api/v1/razorpay/payments")
        assert response.status_code == 200
        body = response.json()
        assert body["count"] == 1 and body["limit"] == DEFAULT_PAGE_SIZE
        item = body["items"][0]
        for field in ("external_id", "amount_minor", "currency", "status",
                      "order_id", "method", "captured", "fee_minor",
                      "tax_minor", "created_at"):
            assert field in item
        assert item["fee_minor"] == 588 and item["tax_minor"] == 89
        assert item["created_at"].startswith("2025-08-23T00:00:00")
        assert item["provider"] == "razorpay"
    ...
    def test_empty_collection_returns_empty_items(
        ...
        body = api.get("/api/v1/razorpay/payments").json()
        assert body == {"items": [], "count": 0, "limit": DEFAULT_PAGE_SIZE,
                        "offset": 0}


class TestPaymentsEndpointFailures:
        ...
        def handler(request):
            return httpx.Response(upstream_status, json={
                "error": {"code": "BAD_REQUEST_ERROR",
                          "description": "auth failure details"}})
    ...
    def test_not_found_passthrough_404(self, api, install_service) -> None:
        ...
        assert api.get("/api/v1/razorpay/payments").status_code == 404
...
class TestPaymentsParameterValidation:
    @pytest.mark.parametrize(
        "query",
        [
            "?count=0",
    ...
    def test_invalid_queries_yield_422_without_upstream_call(
        ...
        install_service(make_service(handler))
        response = api.get(f"/api/v1/razorpay/payments{query}")
        assert response.status_code == 422
        assert called is False


class TestPaymentsOpenApi:
    def test_path_and_failure_responses_documented(self, api) -> None:
        schema = api.get("/openapi.json").json()
        entry = schema["paths"]["/api/v1/razorpay/payments"]["get"]
    ...
    def test_response_schema_exposes_no_credential_fields(self, api) -> None:
        schema = api.get("/openapi.json").json()
        props = schema["components"]["schemas"]["NormalizedPayment"][
            "properties"
        ].keys()
